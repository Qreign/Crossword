...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for X in self.domains[x]:
            ...
            self.domains[x] -= rm
        print(f'Revisions Made: {revised}, Revise completed successfully')
        return revised
        ...
        while arcs:
            ...
            if self.revise(x, y):
                if not self.domains[x]:
                    print("AC-3 could not be implemented")
                    return False
                arcs += [(z, x) for z in self.crossword.neighbors(x) - {y}]
        print(f"AC-3 has been implemented. Iterations")  
        return True
    ...
    def consistent(self, assignment):
        ...
        word_list = set()
        for X in assignment:
            if assignment[X] not in word_list:
                word_list.add(assignment[X])
            else:
                return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        for X in self.domains[var]:
            order[X] = 0
            for Y in self.crossword.neighbors(var) - set(assignment.keys()):
